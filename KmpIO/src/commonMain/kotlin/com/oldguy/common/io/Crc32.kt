...
/**
 * Simple Kotlin-only CRC calculation for use on any platform. No expect/actual setup.
 * Repeatedly call update with inputs until done, get CRC from result property.
...
class Crc32() {
    ...
    private fun tableLookup(input: ByteArray) {
        for (b in input) {
            value = value ushr 8 xor table[value xor b.toInt() and 0xff]
        }
    }

    private fun tableLookup(input: UByteArray) {
        for (b in input) {
            value = value ushr 8 xor table[value xor b.toInt() and 0xff]
        }
    }
